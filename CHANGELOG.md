...
### Removed
- Removed the redundant `commons-io.version` property: it repeated the value `jeap-internal-spring-boot-parent` already sets. The two modules using commons-io now take the version from the parent's dependency management.
- Removed the explicit versions on `jeap-messaging-avro`, `-avro-compiler` and `-avro-validator`: all used the parent's own `${jeap-messaging.version}` for artifacts the parent already manages.
...
- **org.eclipse.jgit:org.eclipse.jgit.ssh.apache**: 7.7.1.202607240634-r → 7.8.0.202609011348-r (minor)
...
### Changed
- The message version status endpoint now checks the registry default branch and skips unavailable version information with a warning.

## [10.1.1] - 2026-08-17

### Changed
- Consolidated semantic-version parsing used by contract status and Renovate compatibility checks.

## [10.1.0] - 2026-08-17

### Changed
- Added a read-authenticated endpoint that reports whether deployed contracts use the latest registry message versions.
...
- Use shallow clone and per-checkout fetch when loading message type registries, reducing data transferred from the git
  remote
- Fix duplicate errors when uploading multiple contracts
